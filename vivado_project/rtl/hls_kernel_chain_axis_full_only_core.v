...
`ifdef FFN_FULL_DIAG
    ...
    initial begin
        if (!$value$plusargs("DUMP_DIR=%s", ffn_diag_dir)) begin
            ffn_diag_dir = "sim_dumps/ffn_full_diag_default";
        end
...
`endif
    ...
    initial begin
        $readmemh("q_mult_q30.mem", q_mult_q30_rom);
        $readmemh("k_mult_q30.mem", k_mult_q30_rom);
        $readmemh("v_mult_q30.mem", v_mult_q30_rom);
        $readmemh("attn_proj_mult_q30.mem", attn_proj_mult_q30_rom);
        $readmemh("ffn_mid_mult_q30.mem", ffn_mid_mult_q30_rom);
        $readmemh("ffn_mult_q30.mem", ffn_mult_q30_rom);
        $readmemh("lm_head_weights_padded64.mem", lm_head_weight_rom);
        $readmemh("lm_head_scale_ratio_q30.mem", lm_scale_ratio_rom);
        $readmemh("attn_exp_neg_q6_q4.mem", attn_exp_q6_rom);
    end
